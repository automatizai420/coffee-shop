  ...
  def product_card(assigns) do
    ...
    <div class="rounded-lg overflow-hidden h-full flex flex-col transition-all duration-300 hover:scale-[1.02] group">
      ...
      <div class="p-6 sm:p-4 flex-grow flex flex-col bg-transparent">
        <!-- Nombre del producto -->
        <h3 class="text-xl sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-2"><%= @product.name %></h3>
        
        <!-- Descripción -->
        <p class="text-gray-700 text-base sm:text-sm line-clamp-2 flex-grow">
          <%= @product.description %>
        </p>
        
        <!-- Botón con estilo destacado -->
        <div class="mt-6 sm:mt-4 text-right">
          <.link 
            navigate={~p"/products/#{@product.id}"} 
            class="inline-block text-base sm:text-sm font-medium text-amber-600 hover:text-amber-800 transition-colors underline underline-offset-4 decoration-1"
          >
            Ver detalles
          </.link>
        </div>
